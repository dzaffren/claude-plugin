...
| O7 | The PR's own review found seven more rules the contract stated and the checker did not hold you to — a half-written count line, an overlong verdict, a loose decision match, a summary hidden before the Glossary, non-consecutive numbering, a severity label mid-line, and a finding block that ran to the end of the file | flag | ship | user | Resolved | All seven fixed and re-verified, 2026-09-15. `/review`'s shell permission narrowed to `Bash(bash *check-report.sh*)` in the same pass |
