...
| O8 | `verify-ship-gates.sh`'s placeholder check fails this very spec — `{type}`, `{scope}`, `{subject}` and `{slice}` are the convention, not unfilled blanks. The plan did not anticipate it. | flag | build | model | Resolved | Turned out to be wider than the four names: the rule also fires on every spec's mermaid `{{node}}` syntax and on the gherkin in its acceptance criteria. First attempt blanked fenced blocks and inline code before scanning. Review found four defects in that one hunk — an odd fence count blanked the rest of the spec, a lone backtick swallowed a real placeholder, `TODO` and `[TBD]` inside a fence stopped being caught at all, and an indented fence was still flagged. Two of those were regressions against the pre-diff code. Reverted to the plain grep with the documented tokens added to the existing exclusion list, which is what the surrounding code already does for `{token}` and `{N}`. A bare `{owner}` in prose still fails; so does a `TODO` in a fence. 2026-09-12 |
| O9 | Review found `block-attribution.sh` failing open on `git commit \` + newline + `-m "..."`, the ordinary way to wrap a long commit command. The root cause is in `lib/git-command.py`, which belongs to the shipped `hook-command-match` slice, not to this one. | flag | review | user | Resolved | Fixed at the root, on the user's call: `tokenise()` now collapses backslash-newline before splitting. Same bug was letting a wrapped force-push past `block-dangerous.sh` — a pre-existing hole this diff did not introduce and now closes. Both directions covered by fixtures. Widens the slice by one file beyond the Changes table. 2026-09-12 |
| O10 | The Changes table asked for the type list in `build/SKILL.md` and the ban list in `ship/SKILL.md`, which contradicts the Interface section's "the convention itself is never restated in a skill". | flag | review | model | Resolved | The two restatements drifted inside this diff before it ever shipped — build's ban list omitted `Claude-Session` and `Generated with Claude Code`. Both enumerations removed; each skill now names the format and points at `references/git-naming.md` for the lists. The Interface section wins over the Changes table. 2026-09-12 |
