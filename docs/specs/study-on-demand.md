...
| `plugins/zuko/scripts/tests/run.sh`, after the loop | Exit 1 with `no tests ran, every one was skipped` when nothing recorded a result. | A suite whose every test was skipped would otherwise exit 0 with an empty tally (`gate-scanned-nothing-is-not-a-pass`) |
| `plugins/zuko/scripts/tests/run.sh`, `expect_match` / `expect_no_match` | Both go through a new `grep_text`, which retries when grep's exit code is neither 0 nor 1 and records "the check never happened" rather than a content miss. Text goes in by herestring, never a pipe. | O14 and R2: a grep that could not fork was read as "not found", and piping into `grep -q` turned a real match into SIGPIPE |
| `plugins/zuko/scripts/verify-ship-gates.sh`, `tests/test-verify-ship-gates.sh` | The placeholder rule's exclusion list gains `{topic}`, `{stage}`, `{name}` and `TODO(study)`, with two cases: study's tokens pass, a real blank beside them still fails. | O23: the gate read this slice's own documented formats as unfilled blanks |
...
| **Rollout** | No flag. The skill does nothing until `/study` is typed or a study request routes to it. Version 2.3.0 (O10). The installed copy lags the repo, so `/plugin update zuko@dzafran-claude-plugins` and then a new session (lessons `installed-plugin-lags-the-repo`, `skills-load-at-session-start`). Rollback: `git revert` the merge commit, or pin 2.2.0, the last release without `/study`. |
...
| O11 | The test plan gave sessions B and C a single first turn that starts the mode, answers the level questions and asks for the work. Scenario 1 requires the first reply to be the start line plus the two questions and nothing else, so the two cannot both hold. The first live run failed on exactly this. | flag | build | claude | Resolved | Split both: turn 1 starts the mode, turn 2 does the work. B is now 3 turns, C is 3 turns. Scenario 1's own rule is the one that stands. |
| O12 | The e2e read only the `result` event, which carries just the final assistant message. A fixed line printed before a stretch of tool calls was invisible, so `Study mode off.` was scored missing on two runs when it may well have been printed. | flag | build | claude | Resolved | The driver now collects every assistant text block in the turn. `turn_first` is the first block, for the checks that care about what the reply opens with. |
| O13 | The e2e checked that a hint never contains the string `FileNotFoundError`. Python chains the original exception under the `NameError`, so the quoted traceback names it no matter how good the hint is — and scenario 3 tells the skill to quote the error. The check demanded something the fixture makes impossible. | flag | build | claude | Resolved | Check for the corrected line `except FileNotFoundError` instead. That is scenario 3's actual wording, "without writing the fix". Reading the name out of the traceback is the lesson, not a leak. |
| O14 | `expect_match` could not tell "grep ran and found nothing" from "grep never started". With three live sessions running at once the machine drops to about 3 GB available, `grep` intermittently fails to fork, and the shell reports it as a clean miss. Seen twice in the free suite and once in a live run, each time on text that was plainly present. | flag | build | claude | Resolved | `run.sh` gained `grep_text`, which retries when the exit code is neither 0 nor 1 and records "the check never happened" if it still cannot run. A check that could not run is a failure that says so, never a content failure. |
| O15 | The "reply starts `Not yet.`" check read the first text of the whole turn. A turn that runs the user's code opens with a preamble before the tool call ("Checking what you wrote and running it."), so the verdict is never the first thing. The check failed on a run whose verdict did open with `Not yet.` | flag | build | claude | Resolved | The driver now also exposes `turn_verdict`, the first text after the last tool call. A verdict on an attempt cannot be printed before the attempt has been run. |
| O16 | Review found the e2e could report green while proving nothing. Five checks were written `if grep_text ...; then FAIL; else PASS`, so a grep that never ran scored a pass — the O14 failure inverted. `A3` passed when turn 2 left no gap to hand back, `A5`/`C3` passed when the verdict block parsed empty, and a session killed partway left a short results file that read as a clean run. | flag | review | reviewer | Resolved | `present` / `absent` helpers record FAIL on "could not run". An empty verdict and an absent marker are failures. Each session declares its assertion count and the replay fails if it is short. |
| O17 | `--add-dir` grants writes, not just reads. Pointed at the real `plugins/zuko`, a session under test could edit the skill it was being tested against, in the tracked working tree, and the next session would load the change. Confirmed by experiment: a headless run with `--add-dir` and `--permission-mode acceptEdits` overwrote a file in the added directory. The Non-functionals row claimed the test was confined to `mktemp -d`. | flag | review | reviewer | Resolved | The plugin is copied into the scratch dir and `--plugin-dir` / `--add-dir` point at the copy. |
| O18 | The `grep_text` added for O14 piped text into `grep -q`, which stops reading at the first match; the writer takes SIGPIPE and `pipefail` surfaces 141, so a real match on text above the pipe buffer was reported as "grep could not run". Measured: 400 KB containing the pattern failed. | flag | review | reviewer | Resolved | Herestring instead of a pipe. `test-study-wiring.sh` now checks match, miss and cannot-run on a 400 KB string, and that both helpers record a failure when the check never ran. |
| O19 | `test-study-wiring.sh` could not tell a named live run that executed and failed from one that never ran — exit 1 and no skip line are true of both. A mutation making a named run skip silently passed 13 of 13. | flag | review | reviewer | Resolved | It now asserts the named run shows the live test's own output. The same mutation is caught. |
| O20 | Four rules in `SKILL.md` contradicted other rules in the same file: "quote the error" against "never write the right name" (the traceback contains it), `Still studying.` both ending the reply and carrying on, the opening reply missing from the list of replies with no `Your turn` block, and the failed-attempt branch demanding an error a decision cannot produce. `Your turn` and `Glossary` both claimed the end of the reply. | flag | review | reviewer | Resolved | The prose prohibition is separated from the quoted error, which is always quoted in full. The no-`Your turn` list names all four replies. The failed branch splits code from decision. The order is stated: `Your turn` first, `Glossary` last. |
| O21 | Three live sessions at once left this machine near its memory limit, which is what made `grep` fail to fork. The spec's test plan called for parallel sessions. | flag | review | user | Resolved | Run them serially. The user's call, 2026-09-17: roughly double the wall time, same cost, and it removes the pressure rather than coping with it. |
| O22 | Session C's final turn answered a three-part `Decide:` block — collisions, guessability, two API servers — with a reason covering only collisions, then asserted the reply must not open `Not yet.` The skill correctly judged the answer incomplete, so the check was asserting that a half-answer gets waved through. | flag | review | claude | Resolved | The fixture answers all three dimensions and names the cost it accepts. `Not yet.` on that is a real failure, so the check now tests the behaviour it claims to. |
| O23 | The ship gate's placeholder rule flagged 17 lines of this spec. All four tokens — `{topic}`, `{stage}`, `{name}` and the `TODO(study)` marker — are formats the slice writes down on purpose, the same category as `{type}/{slice}`. The rule cannot tell a documented format from an unfilled blank, which is why it carries a list. Fixing it means changing a file the plan did not name. | flag | ship | claude | Resolved | Added the four to the list, and diffed old against new over every spec in the repo: this spec 17 flagged lines to 0, the other three unchanged at 0. A new case proves a real blank beside those tokens is still caught and named. |
| O24 | The PR reviewer caught that O23's fix dropped the whole line whenever it held a documented token, so `TODO(study): implement {reviewer}` passed clean. Verified as a regression O23 introduced: the old rule flagged that line, the new one did not. The same shape with older tokens was already being waved through, so the line-level design predates the change, but adding a token that appears ten times in this spec widened it. The test missed it by putting the blank on a different line. | flag | ship | reviewer | Resolved | Documented tokens are cut out of the line and whatever remains is judged, which closes the older hole too. Three cases added: a blank sharing a line with `TODO(study)`, a blank sharing a line with `{type}/{slice}`, and a line of nothing but documented tokens. |
