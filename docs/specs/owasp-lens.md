...
| `run.sh`'s `expect_match` pipes text into `grep -q` under `pipefail`: on text over 64 KB, `printf` dies of SIGPIPE, a match reads as a miss, and `expect_no_match` passes whatever the text says | The checks on the 94 KB spec grep the file directly (`in_spec`, `not_in_spec` in `test-owasp-lens.sh`). Fixing `run.sh` is a separate slice |

### Decisions to record

Recorded as D4, D5, D6, D7, D8, D9 (D9 supersedes D8).
