...
| O13 | Run 6: scenario 4 fails at the reviewer, not the verifier. The reviewer covering `auth.py` listed A09 as checked and wrote "login() and the session handling are unchanged", though the diff deletes the failed-login `log.warning` from `login()`. Runs 2 to 5 raised it every time; the seeded run is one sample (O2). Options: accept it as the variance O2 already accepted and mark the slice Built on runs 5 and 6 together; or make the reviewer's A09 check read the removed lines (`git diff` lines starting with `-`) of every auth function first, which 7b's regression-aware review also covers | flag | build (run 6) | user | Open | — |
